...
function useDebounceFn(func, delay, options = { leading: false, trailing: true }) {
    ...
    const funcRef = useFreshCallback.useFreshCallback(func);
    const [isTimeoutEnabled, setIsTimeoutEnabled] = react.useState(false);
    // we use key to reset a timeout when the key changes
    const [key, setKey] = react.useState(0);
    const lastExecutionTimeRef = react.useRef(0);
    const argsRef = react.useRef();
    const debouncedFn = react.useCallback((...args) => {
    ...
    const freshDebouncedFn = useFreshCallback.useFreshCallback(debouncedFn);
